...
unsigned short LOD = 0x0; // load *					-> Load register -- LOD rN or LOD
unsigned short STO = 0x1; // store *    			-> Store register -- STO rN or STO
unsigned short ADD = 0x2; // add *   	  			-> Add r0/A and memory
unsigned short SUB = 0x3; // subtract * 			-> Subtract r0/A and memory
unsigned short ADR = 0x4; // add indirect * 		-> Add rN and memory
unsigned short SUR = 0x5; // subtract indirect * 	-> Subtract rN and memory
unsigned short AND = 0x6; // logical AND *			-> Logical AND rN and memory
unsigned short IOR = 0x7; // logical OR *			-> Logical OR rN and memory
unsigned short NOT = 0x8; // logical NOT *			-> Logical Negate rN
unsigned short JMP = 0x9; // jump					-> Unconditionally jumps to rN
unsigned short JEQ = 0xA; // jump EQUAL **			-> Jumps to rN if and only if
unsigned short JGT = 0xB;
unsigned short JLT = 0xC;
unsigned short CMP = 0xD;
unsigned short CLR = 0xE;
unsigned short HLT = 0xF;
